...
public class HyperbolicUtils {
    ...
    /**
     * Compute the Busemann function B_xi(x) in the Poincaré ball model.
     * xi must be on the boundary (||xi|| = 1).
     */
    ...
    /**
     * Gradient of the Busemann function B_xi(x) with respect to x in the Poincaré ball model.
     * xi must be on the boundary (||xi|| = 1).
     */
    ...
    public static double acosh(double x) {
        if (x < 1.0) throw new IllegalArgumentException("acosh is undefined for x < 1");
    ...
    public static VectorN mobiusAdd(VectorN x, VectorN y) {
        ...
        if (denominator == 0.0) throw new ArithmeticException("Möbius addition division by zero");
    ...
    public static VectorN mobiusLogMap(VectorN x, VectorN y) {
        ...
        if (normDiff == 0.0) return new VectorN(x.dimension());
